...
当前模型是可复现的规则 baseline，用于验证链路，不是最终 LightGBM 模型。模型产物已经记录时间序列评估口径，包括 5 个交易日标签 gap、train/valid/test 评估窗口、基础收益/动量/波动率/RSI/MACD 因子和 baseline 测试指标，后续接入 LightGBM 时沿用同一元数据结构。
